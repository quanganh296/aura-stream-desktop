...
const Login = () => {
  ...
  const handleSocialAuth = async (provider, customData = {}) => {
    ...
    setErrorMsg('');
    try {
      await socialLogin(provider, customData);
      navigate('/home');
    } catch (err) {
    ...
    <div className="auth-page-container">
      ...
      <div className="auth-wrapper">
        ...
        <div className="auth-card glass-panel">
          ...
          <div className="social-grid">
            <button className="btn-social-auth" type="button" title="Google" onClick={() => setActiveOAuthProvider('google')}>
            ...
            <button className="btn-social-auth" type="button" title="Facebook" onClick={() => setActiveOAuthProvider('facebook')}>
              <svg className="social-svg" viewBox="0 0 24 24">
                <path fill="#1877F2" d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
              </svg>
            </button>
            <button className="btn-social-auth" type="button" title="Apple" onClick={() => setActiveOAuthProvider('apple')}>
            ...
            <button className="btn-social-auth" type="button" title="X (Twitter)" onClick={() => setActiveOAuthProvider('x')}>
              <svg className="social-svg" viewBox="0 0 24 24" fill="#ffffff">
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
              </svg>
            </button>
            <button className="btn-social-auth" type="button" title="Twitch" onClick={() => setActiveOAuthProvider('twitch')}>
      ...
      {activeOAuthProvider && (
        <OAuthModal
          ...
          onConfirm={(prov, customData) => handleSocialAuth(prov, customData)}
          onCancel={() => setActiveOAuthProvider(null)}
        />
      )}
    </div>
  );
